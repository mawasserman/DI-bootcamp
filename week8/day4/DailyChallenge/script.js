...
const getCarHonda = inventory.filter(honda => honda.car_make == "Honda");
console.log(getCarHonda);
const first = getCarHonda[0];
console.log(`This is a ${first.car_make} ${first.car_model} from ${first.car_year}`);
...
// Hint : Check out this tutorial on the sort method --------------used this hint
// Use the cars inventory below:

const sortCarInventoryByYear = inventory.sort(function (x, y){
    let a = new Date(x.car_year),
    b = new Date(y.car_year);
    return a - b;
});

console.table(sortCarInventoryByYear);
