...
SELECT film_id, title, description, length, rental_rate FROM film WHERE title = 'Braveheart'; 
-- :(
...
SELECT title FROM film ORDER BY rental_rate
OFFSET 10 ROWS
...
SELECT film.title, film.film_id FROM film WHERE film.film_id NOT IN (SELECT film_id FROM inventory )
